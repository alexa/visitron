...
eval_iters=""
for (( COUNTER=810; COUNTER<=50000; COUNTER+=100 )); do  # 10 to 40010 gap 500
    eval_iters="$eval_iters $COUNTER"
done

# eval_iters="$eval_iters 10000"

exp_name=viewpoint/ndh_r2r-pretrain-ndh-r2r-diff-seed

arguments="
--img_feat_dir srv/img_features
--img_feature_file ResNet-152-imagenet-pytorch.tsv
--data_dir srv/task_data/NDH/data
--model_name_or_path srv/results/$exp_name/checkpoints/
--output_dir srv/results/$exp_name-val
