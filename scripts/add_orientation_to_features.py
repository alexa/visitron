...
start = time.time()
new_data = load_tsv_features_and_change_data_types(f"img_features/{FILE}.tsv")
now = time.time()
print("Time taken for loading tsv file: %0.4f mins" % ((now - start) / 60))

start = time.time()
with open(
    f"img_features/{FILE}.pickle",
    "wb",
) as f:
...
start = time.time()
new_data = load_pickle_features_and_add_orientation(f"img_features/{FILE}.pickle")
now = time.time()
print(
...
start = time.time()
with open(
    f"img_features/{FILE}-worientation.pickle",
    "wb",
) as f:
    pickle.dump(new_data, f)
now = time.time()
