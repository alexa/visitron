...
class BertImgModelwithAction(BertPreTrainedModel):
    ...
    def __init__(self, config):
        ...
        self.action_embedding = nn.Embedding(8, self.config.hidden_size)
    ...
    def forward(
        ...
        embedding_output = torch.cat(
            (embedding_output, img_embedding_output, action_embedding_output), 1
        )
        ...
        if hasattr(config, "classifier"):
            ...
            if config.classifier == "linear":
                ...
                self.classifier = nn.Sequential(
                    nn.Linear(
                        config.hidden_size, config.hidden_size * config.cls_hidden_scale
                    ),
                    nn.ReLU(),
                    nn.Linear(
