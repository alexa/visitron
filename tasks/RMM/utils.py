...
def load_datasets(
    splits, datasets="CVDN", mount_dir="", segmented=False, speaker_only=False
):
    data = []
    for split in splits:
        ...
        for dataset_type in datasets.split("_"):
            ...
            if dataset_type == "R2R":
                data_source += (
                    "tasks/" + dataset_type + "/data/" + dataset_type + "_%s.json"
                )
            else:
                        ...
                        item["dialog_history"] = [
                            {
                                "nav_idx": 0,
                                "role": "navigator",
                                "message": "I am lost . where should I go next ?",
                            },
                            {
                                "nav_idx": 0,
                                "role": "oracle",
                                "message": item["instructions"][0],
                            },
                        ]
                    ...
                    for item in new_items:
                        ...
                        if not speaker_only:
                            if first_nav_index > 0:
                                ...
                                spread_out_item["inst_idx"] = (
                                    item["inst_idx"] + "_" + str(spread_counter)
                                )
                                ...
                                spread_out_item["player_path"] = item["nav_history"][
                                    : first_nav_index + 1
                                ]
                                spread_out_item["planner_path"] = spread_out_item[
                                    "player_path"
                                ]
                                spread_out_item["end_panos"] = [
                                    item["nav_history"][first_nav_index]
                                ]
                            ...
                            if turn["role"] == "navigator":
                                ...
                                if next_turn["role"] != "navigator":
                                    for j in range(index + 1, len_dialog_history):
                                        ...
                                        if (
                                            next_next_turn["role"] == "navigator"
                                            or j + 1 == len_dialog_history
                                        ):
                                            spread_out_item = dict(item)
                                            end_idx = (
                                                int(next_next_turn["nav_idx"]) + 1
                                                if j + 1 < len_dialog_history
                                                else len_path
                                            )  # To next NAV or goal
                                            if (
                                                end_idx - nav_idx < 2
                                            ...
                                            spread_out_item["player_path"] = item[
                                                "nav_history"
                                            ][nav_idx:end_idx]
                                            spread_out_item[
                                                "planner_path"
                                            ] = spread_out_item["player_path"]
                                            # spread_out_item['start_pano'] = {'heading': item['start_pano']['heading'],'elevation': 17.5,'pano':spread_out_item['player_path'][0]}
                                            spread_out_item["end_panos"] = [
                                                item["nav_history"][end_idx - 1]
                                            ]
                                            spread_out_item["nav_instructions"] = turn[
                                                "message"
                                            ]
                                            spread_out_item[
                                                "ora_instructions"
                                            ...
                                            spread_out_items.append(spread_out_item)
                                            nav_histories.append(
                                                spread_out_item["player_path"]
                                            )
                                            spread_counter += 1
                                            exchange_points.add(nav_idx)
                                            turn_items += (
                                                len(spread_out_item["player_path"]) - 1
                                            )
                                            break
...
class GPTTokenizer(object):
    ...
    def __init__(
        self, encoding_length=20, added_special_tokens=["<NAV>", "<ORA>", "<TAR>"]
    ):
    ...
    def encode_sentence(self, sentences, seps=None, get_mask=False):
        ...
        encoding_length = self.encoding_length - 2  # for <BOS> and <EOS>
        ...
        tokens = tokens[-encoding_length:]

        encoding = self.tokenizer.convert_tokens_to_ids(tokens)

        # Add <BOS> to start and <EOS> to end
        encoding = (
            [self.tokenizer.bos_token_id] + encoding + [self.tokenizer.eos_token_id]
        )
        mask = [1] * len(encoding)

        if len(encoding) < self.encoding_length:
            mask += [0] * (self.encoding_length - len(encoding))
            encoding += [self.tokenizer.pad_token_id] * (
                self.encoding_length - len(encoding)
            )

        if get_mask:
            return np.array(encoding), np.array(mask)
    ...
    def shrink(self, inst):
        ...
        # return self.tokenizer.decode(token_ids=inst, skip_special_tokens=True, clean_up_tokenization_spaces=True)
...
def load_features(feature_store, blind, debug=False):
    features = None
    image_w = 640
    image_h = 480
    vfov = 60
...
def get_point_angle_feature(baseViewId=0, angle_feat_size=4):
    ...
    for ix in range(36):
        if ix == 0:
            sim.newEpisode(
                "ZMojNkEp431", "2f4d90acd4024c269fb0efe49a8ac540", 0, math.radians(-30)
            )
        elif ix % 12 == 0:
...
def viewpoint_drop_mask(viewpoint, seed=None, drop_func=None):
    ...
    drop_mask = drop_func(torch.ones(2048).to(args.device))
    return drop_mask
