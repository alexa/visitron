...
class SpeakerEncoder(nn.Module):
    def __init__(
        self, feature_size, hidden_size, dropout_ratio, bidirectional, featdropout=0.3
    ):
...
class GPT2SpeakerDecoder(nn.Module):
    def __init__(
        self, config, tokenizer, encoder_hidden_dim, padding_idx, dropout_ratio
    ):
        from transformers import GPT2Model

        super(GPT2SpeakerDecoder, self).__init__()

        self.gpt2 = GPT2Model.from_pretrained("gpt2", config=config)

        new_vocab_length = len(tokenizer)
        self.gpt2.resize_token_embeddings(new_vocab_length)

        self.encoder_hidden_size = encoder_hidden_dim
        ...
        self.drop = nn.Dropout(dropout_ratio)
        self.attention_layer = SoftDotAttention(
            self.hidden_size, self.encoder_hidden_size
        )
        self.projection = nn.Linear(self.hidden_size, config.vocab_size)
        self.baseline_projection = nn.Sequential(
    ...
    def forward(self, words, ctx, ctx_mask, words_mask=None, past_key_values=None):
        outputs = self.gpt2(
            input_ids=words, attention_mask=words_mask, past_key_values=past_key_values
        )
        ...
        x, _ = self.attention_layer(
            ...
            .expand(-1, multiplier, -1, -1)
            .contiguous()
            .view(batchXlength, -1, self.encoder_hidden_size),
            mask=ctx_mask.unsqueeze(1)
            .expand(-1, multiplier, -1)
            .contiguous()
            .view(batchXlength, -1),
