...
class BaseAgent(object):
    ...
    def test(self):
        ...
        while True:
            if self.turn_based:
                trajs, gen_dialogs = self.rollout(
                    train=False, speaker_branching=self.eval_branching
                )
...
class Seq2SeqAgent(BaseAgent):
    ...
    def __init__(
        ...
        self.criterion = nn.CrossEntropyLoss(
            ignore_index=self.model_actions.index("<ignore>"), reduction="none"
        )
    ...
    def n_outputs():
        return (
            len(Seq2SeqAgent.model_actions) - 2
    ...
    def _sort_batch(self, obs):
        ...
        return (
            Variable(sorted_tensor, requires_grad=False).long().to(args.device),
            mask.byte().to(args.device),
            Variable(seq_lengths, requires_grad=False).long().to(args.device),
            list(perm_idx),
        )
    ...
    def _feature_variable(self, obs):
        ...
        return Variable(torch.from_numpy(features), requires_grad=False).to(args.device)
    ...
    def _teacher_action(self, obs, ended):
        ...
        return Variable(a, requires_grad=False).to(args.device)
    ...
    def rollout(
        ...
        loss = torch.zeros(len(obs)).to(args.device)
        ...
        for t in range(start_t, episode_len):
            ...
            if self.critic is not None:
                probs, log_probs = F.softmax(logit, 1), F.log_softmax(
                    logit, 1
        ...
        if self.use_rl and rl_loss is None:
            ...
            rl_loss = Variable(rl_loss, requires_grad=False).to(args.device)
