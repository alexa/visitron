...
class BaseAgent(object):
    ...
    def write_results(self):
        ...
        if len(self.generated_dialog) > 0:
            output = [{'inst_idx': k, 'trajectory': v, 'num_instrs': int(len(self.generated_dialog[k])/2)} for k, v in self.results.items()]
        else:
            output = [{'inst_idx': k, 'trajectory': v, 'num_instrs':0} for k, v in self.results.items()]
