        ...
        for feedback in ["sample"]:
            ...
            for history in ["none", "target", "oracle_ans", "nav_q_oracle_ans", "all"]:
                for blind in [True, False]:
                    ...
                    if path_len is None:
                        if blind:
                            log = "%s-seq2seq-%s-%s-%s-imagenet-blind-log.csv" % (
                                eval_type,
                                history,
                                path_type,
                                feedback,
                            ...
                            log = "%s-seq2seq-%s-%s-%s-imagenet-log.csv" % (
                                eval_type,
                                history,
                                path_type,
                                feedback,
                        ...
                        if blind:
                            log = "%s-seq2seq-%s-%s-%d-%s-imagenet-blind-log.csv" % (
                                eval_type,
                                history,
                                path_type,
                                path_len,
                            ...
                            log = "%s-seq2seq-%s-%s-%d-%s-imagenet-log.csv" % (
                                eval_type,
                                history,
                                path_type,
                                path_len,
                    ...
                    if os.path.isfile(fn):
                        ...
                        metrics = (
                            [
                                ...
                                "val_unseen dist_to_end_reduction",
                            ]
                            if eval_type == "val"
                            else [
                                "test success_rate",
                        ...
                        if len(dfs[log]) == 200:
                            for cond in ["val_seen", "val_unseen", "test"]:
                                ...
                                if ifm in dfs[log]:
                                    summary[cond][abl]["if"][path_type] = list(
                                        dfs[log][ifm]
                                    )
                                gdm = "%s dist_to_end_reduction" % cond
                                if gdm in dfs[log]:
                                    summary[cond][abl]["gd"][path_type] = list(
                                        dfs[log][gdm]
                                    )
