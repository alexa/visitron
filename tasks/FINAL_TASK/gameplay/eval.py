...
from utils import dialog_to_string, load_datasets
from utils_data import load_gameplay_data, load_nav_graphs
...
class Evaluation(object):
    ...
    def __init__(
        ...
        if splits[0] == "val_unseen" and results_dir is not None:
            max_len = -float("inf")
            for k in self.gps:
        ...
        with open(output_file) as f:
            for item in json.load(f):
                ...
                if item["inst_idx"] in instr_ids:
                    ...
                    self._score_item(
                        item["inst_idx"], item["trajectory"], item["num_instrs"]
                    )
        assert (
            len(instr_ids) == 0
    ...
    def bleu_score(self, path2inst, for_nav=False, tok=None, use_dialog_history=False):
        from gameplay.bleu import compute_bleu
        ...
        for path_id, inst in path2inst.items():
            ...
            if not use_dialog_history:
                ...
                gen_ints = [tok._index_to_word[word_id] for word_id in inst]
            else:
