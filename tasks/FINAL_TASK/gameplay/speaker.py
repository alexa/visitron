...
class Speaker:
    ...
    def __init__(
        ...
        self.feature_size = self.args.lstm_img_feature_dim
        ...
        # Model
        logger.info(f"VOCAB_SIZE {self.dataloader.dataset.tok.vocab_size()}")
        self.encoder = model.SpeakerEncoder(
            self.args,
            self.feature_size + self.args.angle_feat_size,
            self.args.rnn_dim,
            self.args.dropout,
    ...
    def get_insts(self, for_nav=False):
        ...
        # for _ in range(total // self.dataloader.batch_size + 1):  # Guarantee that all the data are processed
    ...
    def _candidate_variable(self, obs, actions):
        candidate_feat = np.zeros(
            (len(obs), self.feature_size + self.args.angle_feat_size), dtype=np.float32
        )
        ...
        return torch.from_numpy(candidate_feat).to(self.args.device)
    ...
    def gt_words(self, obs, for_nav=False):
        ...
        return torch.from_numpy(seq_tensor).to(self.args.device)
    ...
    def teacher_forcing(
        ...
        if features is not None:
            ...
            ctx = self.encoder(can_feats, img_feats, lengths)
        h_t = torch.zeros(1, batch_size, self.args.rnn_dim).to(self.args.device)
        c_t = torch.zeros(1, batch_size, self.args.rnn_dim).to(self.args.device)
        ctx_mask = utils.length2mask(lengths, device=self.args.device)
    ...
    def infer_batch(
        ...
        word = torch.from_numpy(word).view(-1, 1).to(self.args.device)
    ...
    def load(self, path):
        ...
        logger.info("Load the speaker's state dict from %s at epoch %d" % (path, epoch))
        return epoch
