...
class LSTMAgent(BaseAgent):
    ...
    def __init__(
        ...
        if args.no_pretrained_model:
            ...
            if bert is None and word_embedding is not None:
                enc_hidden_size = args.rnn_dim // 2 if args.bidir else args.rnn_dim
                self.encoder = model_lstm.EncoderLSTMOscar(
                    args,
                    word_embedding,
            ...
            elif bert is not None and word_embedding is None:
                self.encoder = model_lstm.OscarEncoder(
                    args=args,
                    bert=bert,
    ...
    def rollout(self, train=True):
        ...
        for t in range(self.episode_len):
            ...
            h_t, c_t, logit, h1 = self.decoder(
                input_a_t, f_t, candidate_feat, h_t, h1, c_t, ctx, ctx_mask,
            )
