...
from classifier.agent import Agent
from classifier.data_loader import (
    ClassifierDataLoader,
    ClassifierDataloader_collate_fn,
    ClassifierDataset,
)
...
from model_utils import MODEL_CLASS, special_tokens_dict
...
def train(args, features):
    ...
    train_data_loader = ClassifierDataLoader(
        dataset=train_dataset,
        splits=["train"],
        feature_store=features,
        tokenizer=tokenizer,
        batch_size=args.train_batch_size,
        collate_fn=ClassifierDataloader_collate_fn,
    ...
    logger.info("Training an Classifier agent with %s feedback" % args.feedback_method)
    ...
    for idx in range(0, n_iters, log_every):
        ...
        if args.local_rank in [-2, -1, 0]:
            ...
            for metric, value in agent.metrics.items():
                loss_str += ", %s: %.3f" % (metric, value)
                tb_writer.add_scalar(
                    f"classification/{metric}_train", value, global_step=iter_no
                )
...
def val(args, features, list_iter_no):
    ...
    for iter_no in list_iter_no:
        ...
        val_unseen_dataset = ClassifierDataset(
            args=args,
            splits=["val_unseen"],
            tokenizer=tokenizer,
            truncate_dialog=True,
        ...
        for split, val_dataset in val_datasets.items():
            ...
            val_data_loader = ClassifierDataLoader(
                dataset=val_dataset,
                splits=["val_seen"],
                feature_store=features,
                tokenizer=tokenizer,
        ...
        logger.info(
            "Validating the Classifier agent with %s feedback for iteration %d"
        ...
        for env_name, (dataloader, evaluator) in val_data_loaders.items():
            ...
            tb_writer.add_scalar(
                f"classification/loss_{env_name}", val_loss_avg, global_step=iter_no
            )
            data_log["%s loss" % env_name].append(val_loss_avg)

            loss_str = ", %s loss: %.4f" % (env_name, val_loss_avg)
            for metric, val in agent.metrics.items():
                data_log["%s %s" % (env_name, metric)].append(val)
                loss_str += ", %s: %.3f" % (metric, val)
                tb_writer.add_scalar(
                    f"classification/{metric}_{env_name}", val, global_step=iter_no
                )
